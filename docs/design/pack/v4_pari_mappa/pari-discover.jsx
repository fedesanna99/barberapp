...
const PARI_BARBERS_MAP = (typeof DEMO_BARBERS !== "undefined" ? DEMO_BARBERS : []).map((b, i) => {
  // Distribute pins in pleasing positions. Keep clear of:
  //   - top 28% (search bar + agenda pill + filter chips)
  //   - bottom: sheet peek at ~y=64% with 22px buffer
  // So pin centers live in y=30%..58% (after -100% translate, tail bottoms
  // land 0..28% above their center, which is the safe zone).
  const positions = [
    { x: 30, y: 56 },  // Marco — Cagliari centro
    { x: 64, y: 48 },  // Fadi
    { x: 22, y: 70 },  // Nico
    { x: 74, y: 66 },  // Rita
    { x: 46, y: 78 },  // Salvo
...
function V4Pin({ b, selected, onClick }) {
  ...
  return (
    <button
      ...
      {/* selected halo — soft clay pulse behind the pin */}
      {selected && (
        <span style={{
          position: "absolute",
          left: "50%", top: "50%",
          ...
          background: "var(--clay)",
          opacity: 0.25,
          animation: "v4PinPulse 1.6s var(--ease) infinite",
          pointerEvents: "none",
        }}/>
      )}
      <div style={{
...
function PariScreenDiscover({ onOpenBooking, onOpenProfile }) {
  ...
  // browse-mode snap: "min" (handle only, map fullscreen) | "mid" | "full"
  const [snap, setSnap] = useStateD("min");
  ...
  const filtered = useMemoD(() => {
    ...
    if (activeFilter === "Aperti ora") arr = arr.filter(b => b.status === "open");
    else if (activeFilter === "Top rated") arr = arr.filter(b => b.rating >= 4.8);
    else if (activeFilter === "Vicini") arr = arr.filter(b => b.km <= 1.5);
  ...
  // sheet height
  let sheetHeight;
  if (mode === "card") sheetHeight = "auto";
  else if (snap === "min") sheetHeight = 64;
  else if (snap === "mid")  sheetHeight = "55%";
  else                       sheetHeight = "94%";
  ...
  return (
    <div className="bb-screen" style={{
      ...
      <V4MapCanvas>
        {filtered.map(b => (
          <V4Pin key={b.id} b={b}
                 ...
                 onClick={() => { setMode("card"); setSelectedId(b.id); setSnap("min"); }}/>
        ))}
      </V4MapCanvas>
      ...
      {/* TOP STRIP — agenda pill + filter chips in one row.
         Single flex container so the pill and the chips never overlap.
         pill is fixed-width on the left, chips scroll horizontally on the right. */}
      <div style={{
        ...
        <button style={{
          ...
          <span style={{
            ...
            <Icon name="clock" size={11} color="var(--paper-3)"/>
          </span>
          <span>sab <span style={{ fontFamily: "var(--font-mono)", fontWeight: 600 }}>10:00</span></span>
          <Icon name="chevron-right" size={10} color="rgba(252,250,245,0.5)"/>
        </button>

        {/* FILTER CHIPS — scrollable row, hugs the right side */}
        <div style={{
          flex: 1, minWidth: 0,
          display: "flex", gap: 6,
          overflowX: "auto",
          ...
          {PARI_QUICK_FILTERS.slice(0, 4).map(f => {
            ...
            return (
              <button key={f} onClick={() => setActiveFilter(f)} style={{
                flexShrink: 0,
                padding: "6px 11px",
                background: active ? "var(--clay)" : "var(--paper-3)",
      ...
      <div style={{
        ...
        {/* handle + snap toggle (shows count + "Lista" pill when minimal) */}
        <div
          onClick={() => {
          ...
          style={{
            width: "100%",
            padding: snap === "min" ? "14px 20px" : "10px 0 6px",
            display: "flex",
            alignItems: "center",
            justifyContent: snap === "min" ? "space-between" : "center",
            gap: 10,
          ...
          {snap === "min" ? (
            <>
              <span style={{
                ...
                <span style={{ color: "var(--ink-60)", marginLeft: 6 }}>
                  {filtered.length === 0 ? "risultati" : filtered.length === 1 ? "bottega vicino a te" : "bottega vicino a te"}
                </span>
              </span>
              <span style={{
          ...
          {mode === "card" ? (
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 13.5, color: "var(--ink-60)", lineHeight: 1.5, maxWidth: 280 }}>
                  Nessun barbiere corrisponde a "{query.trim() || activeFilter}". Prova a togliere qualche filtro.
                </div>
              </div>
              <button
                onClick={() => { setActiveFilter("Tutti"); setQuery(""); }}
                style={{
                  padding: "10px 18px",
                  background: "var(--ink)", color: "var(--paper-3)",
                  border: "none",
